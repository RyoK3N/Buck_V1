...
class Settings(BaseSettings):
    ...
    claude_model: str = Field("claude-opus-4-5", env="CLAUDE_MODEL")
    claude_thinking_budget: int = Field(10000, env="CLAUDE_THINKING_BUDGET")
    claude_max_iterations: int = Field(10, env="CLAUDE_MAX_ITERATIONS")
    claude_session_token_budget: int = Field(200000, env="CLAUDE_SESSION_TOKEN_BUDGET")

    # --- Accuracy tracking --------------------------------------------------
    accuracy_db_path: str = Field("accuracy/buck_accuracy.db", env="ACCURACY_DB_PATH")
    accuracy_poll_interval_minutes: int = Field(5, env="ACCURACY_POLL_INTERVAL_MINUTES")
    accuracy_scheduler_enabled: bool = Field(True, env="ACCURACY_SCHEDULER_ENABLED")
    market_tz: str = Field("Asia/Kolkata", env="MARKET_TZ")
    market_exchange: str = Field("NSE", env="MARKET_EXCHANGE")
...
def set_anthropic_key(anthropic_api_key: str, env_file: str = ".env") -> None:
    """Persist Anthropic API key to .env and refresh settings."""
    set_key(env_file, "ANTHROPIC_API_KEY", anthropic_api_key)
    os.environ["ANTHROPIC_API_KEY"] = anthropic_api_key
    get_settings.cache_clear()
    global SETTINGS
    SETTINGS = get_settings()
    LOGGER.info("Anthropic API key updated and saved to %s", env_file)
