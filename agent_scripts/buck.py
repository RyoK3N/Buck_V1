...
class Buck:
    ...
    async def analyze_and_predict(
        ...
        try:
            ...
            resolved_model = model_name or self.config['chat_model']
            results = {
                'symbol': symbol,
            ...
            # Step 5: Persist to accuracy DB (best-effort telemetry; never raises)
            self._record_prediction_telemetry(symbol, resolved_model, forecast, results)
    ...
    def _record_prediction_telemetry(
        ...
        """Best-effort write to the accuracy DB. Swallows all exceptions —
        forecast pipeline must not fail just because telemetry can't write."""
        try:
            from accuracy.repository import record_prediction
            # If the predictor recorded a tool-call trace (Claude), surface the
            # tool names so the accuracy dashboard can attribute outcomes.
            trace = getattr(self.predictor, "last_trace", None) or []
            tools_used = (
                [step.get("name") for step in trace if isinstance(step, dict) and step.get("name")]
                if trace
                else self._tools_used_from_analysis(results.get('analysis_results') or [])
            )
            metadata = {
                'data_info': results.get('data_info'),
                'analysis_confidence': results.get('metadata', {}).get('analysis_confidence'),
                'tools_used': tools_used,
                'trace': trace,
            }
            record_prediction(symbol=symbol, model=model, forecast=forecast, request_metadata=metadata)
        except Exception as exc:  # noqa: BLE001
            LOGGER.debug("accuracy telemetry skipped: %s", exc)

    @staticmethod
    def _tools_used_from_analysis(analysis_results: List[AnalysisResult]) -> List[str]:
        names: list[str] = []
        for ar in analysis_results:
            data = ar.get('data') if isinstance(ar, dict) else None
